...
{
    public partial class frmGlovesStore : MetroForm
    {
        #region Variables
        ...
        private void frmStockIssuance_Load(object sender, EventArgs e)
        {
            ...
            if (IsOut)
            {
                this.Text = "Gloves Store Material OutPass";
            }
            else
            {
                this.Text = "Gloves Store Materials InPass";
            }
        ...
        private void grdProducts_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 8)
            {
                ...
                if (GPType == 1 && IsOut)
                {
                    ...
                    {
                        if (Validation.GetSafeLong(grdProducts.Rows[e.RowIndex].Cells["colItemType"].Value) == 3)
                        {
                            if (IdOrder == Guid.Empty || PersonAccountNo == string.Empty)
                            {
                                MessageBox.Show("Proper Order Is Not Selected OR Department Person Is Not Selected....");
                                return;
                            }
                            ProcessQuantity = manager.GetRepairStockClosingStockForAllArticles(Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value), IdOrder, PersonAccountNo);//Manager.GetItemClosingStock(Operations.IdCompany, Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value))[0].Qty;
                            if (ProcessQuantity >= Validation.GetSafeDecimal(grdProducts.Rows[e.RowIndex].Cells["colQty"].Value))
                            {
                                //Allow In Put Here...
                            }
                            else
                            {
                                MessageBox.Show("Only '" + ProcessQuantity+"' Repair Stock Is Available To Issue Repair....");
                    ...
                    {
                        ProcessQuantity = Manager.GetGlovesMaterialStoreStockAndBalance(Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value), Operations.IdCompany)[0].Qty;//Manager.GetItemClosingStock(Operations.IdCompany, Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value))[0].Qty;
        ...
        private void cbxGatePassType_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else if (cbxGatePassType.Text == "Gloves Maker")
            {
                lblArticle.Visible = true;
                txtArticle.Visible = true;
            }
        ...
        private void FillVoucher()
        {
            ...
            if (lstVoucher.Count > 0)
            {
                ...
                if (lstVoucher[0].GatePassType == "Gloves Maker")
                {
                    lblArticle.Visible = true;
                    txtArticle.Visible = true;
                }
