...
namespace Accounts.UI
{
    public partial class frmTotalStock : MetroForm
    {
        ...
        private void btnLoad_Click(object sender, EventArgs e)
        {
            ...
            if (!chkByGlovesCategory.Checked && !chkByGarmentsCategory.Checked)
            {
            ...
            else
            {
                if (chkDate.Checked)
                {
                   lstStock = manager.GetRawMaterialTotalStock(Validation.GetSafeGuid(CbxCategories.SelectedValue), Operations.IdCompany);
                }
                else
                {
                   lstStock = manager.GetDateWiseRawMaterialTotalStock(Validation.GetSafeGuid(CbxCategories.SelectedValue), Operations.IdCompany, StartDate.Value, EndDate.Value);
                }
                if (lstStock.Count > 0)
                {
        ...
        {
            if (chkByGlovesCategory.Checked)
            {
                chkByGarmentsCategory.Checked = false;
                FillCategories("Gloves Raw Materials");
                grdTotalStock.Columns["colRubberOut"].Visible = true;
                grdTotalStock.Columns["colDyingUnits"].Visible = false;
                grdTotalStock.DataSource = null;
            }
            else
            {
        ...
        {
            if (chkByGarmentsCategory.Checked)
            {
                chkByGlovesCategory.Checked = false;
                FillCategories("Garments Raw Materials");
                grdTotalStock.Columns["colRubberOut"].Visible = false;
                grdTotalStock.Columns["colDyingUnits"].Visible = true;
                grdTotalStock.DataSource = null;
            }
            else
            {
