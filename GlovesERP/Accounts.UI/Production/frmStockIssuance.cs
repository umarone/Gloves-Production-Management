    ...
    public partial class frmStockIssuance : MetroForm
    {
        ...
        private void frmStockIssuance_Load(object sender, EventArgs e)
        {
            ...
            if (GPType == 1)
            {
                if (IsNewBatch)
                {
                    ...
                    MessageBox.Show("Please Issue Rubber Cuff In Start Of New Batch...");
                }
            }
            if (GPType == 1)
            {
            ...
            {
                grdProducts.Enabled = true;
                pnlBatches.Enabled = false;
            }
            else
            {                
                pnlBatches.Enabled = true;
            }
        }
        ...
        private void LoadGatePassTypes()
        {
            ...
            else if (GPType == 1 && !IsOut)
            {
                cbxGatePassType.Items.Add("");
                cbxGatePassType.Items.Add("Gloves Maker");
                cbxGatePassType.Items.Add("Cuff Cutting");
        ...
        private void ClearControl()
        {
            ...
            txtBatchCost.Text = string.Empty;
            ProductionBatchNo = 0;
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (ValidateControls())
            {
                ...
                #endregion
                #region Create Batch Entry
                listBatch = CreateNewBatch();
                #endregion
                #region Stock Entries
                if (ValidateRows())
                {
                    ...
                    for (int i = 0; i < grdProducts.Rows.Count - 1; i++)
                    {
                        ...
                        if (!IsOut && GPType == 1 && ProductionType == "Stitching")
                        {
                        ...
                        oelProductionIssuance.ExtraAvailableQuantity = Validation.GetSafeDecimal(grdProducts.Rows[i].Cells["colOpeningUnits"].Value);
                        oelProductionIssuance.IssuedExtraQuantity = Validation.GetSafeDecimal(grdProducts.Rows[i].Cells["colIssueOpening"].Value);
                ...
                if (IdVoucher == Guid.Empty)
                {
                    ...
                    if (infoResult.IsSuccess)
                    {
                        if (IsOut)
                        {
                            ...
                            if (IsNewBatch)
                            {
                                txtBatchNo.Text = infoResult.Number.ToString();
                                txtBatchCost.Text = infoResult.OperationCost.ToString("0.000");
                            }
                        }
                        else
                        {
        ...
        private void btnCompleteBatch_Click(object sender, EventArgs e)
        {
            if (IdBatch != Guid.Empty)
            {
                var Manager = new ProductionBatchesBLL();
                if (MessageBox.Show("Are You Sure To Complete This Batch", "Completing Batch", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
                {
                    if (Manager.CompleteBatch(IdBatch))
                    {
                        MessageBox.Show("Batch Is Completed");
                    }
                }
            }
            else
            {
                MessageBox.Show("Please Select Batch To Complete");
            }
        }
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            frmissuance.IdVoucher = IdVoucher;
        ...
        private void grdProducts_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 14)
            {
                ...
                {
                    var manager = new ItemsBLL();
                    listItemCalculation = Manager.GetExtraItemsAvgValue(Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value));
                    if (listItemCalculation.Count > 0)
                    {
                        if (listItemCalculation[0].Qty > 0)
                        {
                            if (grdProducts.Rows[e.RowIndex].Cells["colIssueOpening"].Value != null && Validation.GetSafeDecimal(grdProducts.Rows[e.RowIndex].Cells["colIssueOpening"].Value) > 0)
                            {
                                if (listItemCalculation[0].Qty >= Validation.GetSafeDecimal(grdProducts.Rows[e.RowIndex].Cells["colIssueOpening"].Value))
                                {
                                    //grdProducts.Rows[e.RowIndex].Cells["colRate"].Value = list[0].AvgRate;
                                    //grdProducts.Rows[e.RowIndex].Cells["colAmount"].Value = list[0].AvgRate * Validation.GetSafeDecimal(grdProducts.Rows[e.RowIndex].Cells["colIssueOpening"].Value);
                                }
                                else
                                {
                                    MessageBox.Show("You Are Exceeding With Available Quantity");
                                    grdProducts.Rows[e.RowIndex].Cells["colIssueOpening"].Value = "";
                                    listItemCalculation[0].AvgRate = 0;
            ...
            else if (e.ColumnIndex == 15)
            {
                ...
                if (GPType == 1 && IsOut)
                {
                    if (cbxGatePassType.Text == "Gloves Repair" && IsOut)
                    {
                        ...
                        {
                            grdProducts.Rows[e.RowIndex].Cells["colRate"].Value = Manager.GetItemsAvgValue(Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value));
                    ...
                    else
                    {
                        ...
                        {
                            if (ProductionType == "Talli Cutting" || ProductionType == "Cuff Cutting" || ProductionType == "Cuff Printing" || ProductionType == "Cuff OverLock" || ProductionType == "Cuff Magzi" || ProductionType == "Cuff Tape")
                            {
                                if (Validation.GetSafeInteger(grdProducts.Rows[e.RowIndex].Cells["colItemType"].Value) == 1)
                                {
                                    ProcessQuantity = Manager.GetItemClosingStock(Operations.IdCompany, Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value))[0].Qty;
                                }
                                else
                                {
                                    if (Validation.GetSafeString(grdProducts.Rows[e.RowIndex].Cells["colIssuanceDept"].Value) == "Cuff Cutting")
                                    {
                                        IdDepartment = 1;
                                    }
                                    else if (Validation.GetSafeString(grdProducts.Rows[e.RowIndex].Cells["colIssuanceDept"].Value) == "Cuff Printing")
                                    {
                                        IdDepartment = 2;
                                    }
                                    else if (Validation.GetSafeString(grdProducts.Rows[e.RowIndex].Cells["colIssuanceDept"].Value) == "Cuff OverLock")
                                    {
                                        IdDepartment = 3;
                                    }
                                    else if (Validation.GetSafeString(grdProducts.Rows[e.RowIndex].Cells["colIssuanceDept"].Value) == "Cuff Magzi")
                                    {
                                        IdDepartment = 4;
                                    }
                                    else if (Validation.GetSafeString(grdProducts.Rows[e.RowIndex].Cells["colIssuanceDept"].Value) == "Cuff Tape")
                                    {
                                        IdDepartment = 5;
                                    }
                                    else
                                    ...
                                    ProcessQuantity = manager.GetGlovesBatchWiseItemClosingStock(Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value), ProductionBatchNo, GPType, IsOut ? 1 : 2, ProductionType, IdDepartment);
                                }
                            }
                            else if (ProductionType == "Stitching")
                            {
                                ...
                                else
                                {
                                    ...
                                    ProcessQuantity = manager.GetStockForStitching(Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value), GPType, IdDepartment);
                                        //.GetGlovesBatchWiseItemClosingStock(Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value), ProductionBatchNo, GPType, IsOut ? 1 : 2, cbxProductionType.Text);
                                }
                            }
                        }
                            ...
                            else
                            {
                                ...
                                if (ProductionType == "Talli Cutting" || ProductionType == "Cuff Cutting" || ProductionType == "Cuff Printing" || ProductionType == "Cuff OverLock" || ProductionType == "Cuff Magzi" || ProductionType == "Cuff Tape")
                                {
                                    if (Validation.GetSafeInteger(grdProducts.Rows[e.RowIndex].Cells["colItemType"].Value) != 1)
                                    {
                                        if (IsNewBatch)
                                        {
                                            if (IsOut && ProductionType == "Cuff Printing")
                                            {
                                                grdProducts.Rows[e.RowIndex].Cells["colRate"].Value = Validation.GetSafeDecimal(grdProducts.Rows[e.RowIndex].Cells["colRate"].Value) + Manager.GetItemsAvgValue(Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value)); 
                                            }
                                            else
                                            {
                                                grdProducts.Rows[e.RowIndex].Cells["colRate"].Value = Manager.GetItemsAvgValue(Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value)).ToString("0.00");
                                            }
                                        }
                                        else
                                        {
                                            AvgProcessValue = manager.GetBatchWiseCuffsCost(Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value), IsOut ? 1 : 2, ProductionBatchNo);
                                                //manager.GetGlovesBatchWiseItemAvgCosting(Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value), ProductionBatchNo, GPType, cbxProductionType.Text);
                                            if (listItemCalculation != null && listItemCalculation.Count > 0)
                                            {
                                                if (Validation.GetSafeString(grdProducts.Rows[e.RowIndex].Cells["colQty"].Value) != string.Empty)
                                                {
                                                    if (Validation.GetSafeDecimal(grdProducts.Rows[e.RowIndex].Cells["colIssueOpening"].Value) > 0)
                                                    {
                                                        grdProducts.Rows[e.RowIndex].Cells["colRate"].Value = (AvgProcessValue + listItemCalculation[0].AvgRate).ToString("0.00");
                                                    }
                                                    else
                                                    { 
                                                        grdProducts.Rows[e.RowIndex].Cells["colRate"].Value = AvgProcessValue.ToString("0.00"); 
                                                    }
                                                }
                                                else
                                                {
                                                    /// We Did this because suppose user issuing only Opening Quantity
                                                    if (listItemCalculation != null && listItemCalculation.Count > 0)
                                                    {
                                                        if (listItemCalculation[0].AvgRate > 0)
                                                        {
                                                            grdProducts.Rows[e.RowIndex].Cells["colRate"].Value = listItemCalculation[0].AvgRate.ToString("0.00");
                                                        }
                                                    }
                                                    else
                                                    grdProducts.Rows[e.RowIndex].Cells["colRate"].Value = "";
                                    ...
                                    else
                                    {
                                        if (listItemCalculation != null &&listItemCalculation.Count > 0)
                                        {
                                            grdProducts.Rows[e.RowIndex].Cells["colRate"].Value = (Manager.GetItemsAvgValue(Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value)) + listItemCalculation[0].AvgRate).ToString("0.00");
                                        }
                                        else
                                        {
                                            grdProducts.Rows[e.RowIndex].Cells["colRate"].Value = Manager.GetItemsAvgValue(Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value)).ToString("0.00");
                                ...
                                else if (ProductionType == "Stitching")
                                {
                                    if (Validation.GetSafeInteger(grdProducts.Rows[e.RowIndex].Cells["colItemType"].Value) != 1)
                                    {
                                        ...
                                        else if (Validation.GetSafeString(grdProducts.Rows[e.RowIndex].Cells["colIssuanceDept"].Value) == "OverLock")
                                        {
                                            IdDepartment = 3;
                                        }
                                        else if (Validation.GetSafeString(grdProducts.Rows[e.RowIndex].Cells["colIssuanceDept"].Value) == "Magzi/Tape")
                                        {
                                            IdDepartment = 4;
                                        }
                                        else
                                        ...
                                        AvgProcessValue = manager.GetGlovesStitchingAvgCosting(Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value), GPType, IdDepartment);
                                            //.GetItemsAvgValue(Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value));
                                        grdProducts.Rows[e.RowIndex].Cells["colRate"].Value = AvgProcessValue.ToString("0.00");
                                    }
                                    else
                                    {
                ...
                {
                    if (ProductionType == "Cuff Cutting" || ProductionType == "Cuff Printing" || ProductionType == "Cuff OverLock" || ProductionType == "Cuff Magzi" || ProductionType == "Cuff Tape")
                    {
                        if (Validation.GetSafeInteger(grdProducts.Rows[e.RowIndex].Cells["colItemType"].Value) == 1)
                        {
                            ProcessQuantity = Manager.GetItemClosingStock(Operations.IdCompany, Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value))[0].Qty;
                        ...
                        {
                            ProcessQuantity = manager.GetGlovesBatchWiseItemClosingStock(Validation.GetSafeGuid(grdProducts.Rows[e.RowIndex].Cells["colIdItem"].Value), ProductionBatchNo, GPType, IsOut ? 1 : 2, ProductionType,1);
                        }
                    }
                    else if (ProductionType == "Stitching")
                    {
                    ...
                    if (Validation.GetSafeDecimal(grdProducts.Rows[e.RowIndex].Cells["colQty"].Value) > ProcessQuantity)
                    {
                        if (!IsOut && ProductionType == "Cuff Cutting" || ProductionType == "Talli Cutting")
                        {

                        }
                        else
                        {
                            MessageBox.Show("You Have Issued : " + ProcessQuantity.ToString() + " Quantity While You Are Recieving More Quantity");
                            grdProducts.Rows[e.RowIndex].Cells["colQty"].Value = "";
                            grdProducts.Rows[e.RowIndex].Cells["colRate"].Value = "";
                            grdProducts.Rows[e.RowIndex].Cells["colAmount"].Value = "";
                        }
        ...
        private void grdProducts_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            ...
            else if (grdProducts.CurrentCellAddress.X == 7)
            {
        ...
        private void FillVoucher()
        {
            ...
            if (lstVoucher.Count > 0)
            {
                ...
                txtBatchNo.Text = lstVoucher[0].ProductionBatchNo.ToString();
        ...
        private List<ProductionBatchesEL> CreateNewBatch()
        {
            List<ProductionBatchesEL> list = new List<ProductionBatchesEL>();
            if (GPType == 1 && IsOut && ProductionType == "Cuff Cutting")
            {
                ProductionBatchesEL oelProductionBatch = new ProductionBatchesEL();
                oelProductionBatch.IdBatch = Guid.Empty;
                oelProductionBatch.IdCompany = Operations.IdCompany;
                oelProductionBatch.BookNo = Operations.BookNo;
                oelProductionBatch.OutWardStatus = ProductionType;
                oelProductionBatch.InWardStatus = "";
                oelProductionBatch.BatchStatus = 1;
                oelProductionBatch.ProductionType = GPType;
                oelProductionBatch.CreatedDateTime = DateTime.Now;

                list.Add(oelProductionBatch);
            }
            return list;
        }
        private void btnSelectBatch_Click(object sender, EventArgs e)
        {
            frmfindBatches = new frmfindProductionBatches();
            frmfindBatches.ProductionType = GPType;
            frmfindBatches.ExecuteFindBatchEvent += new frmfindProductionBatches.FindBatchesDelegate(frmfindBatches_ExecuteFindBatchEvent);
            frmfindBatches.ShowDialog();
            //frmfind.StartPosition = FormStartPosition.Manual;
            //frmfind.Location = new Point(this.btnSelectBatch.Location.X, this.btnSelectBatch.Location.X);
            //frmfind.ShowDialog();
        }
        void frmfindBatches_ExecuteFindBatchEvent(object Sender, ProductionBatchesEL oelProductionBatch)
        {
            IdBatch = oelProductionBatch.IdBatch;
            txtBatchNo.Text = oelProductionBatch.ProductionBatchNo.ToString();
            ProductionBatchNo = oelProductionBatch.ProductionBatchNo;
            grdProducts.Enabled = true;
            GetBatchByid();
        }
        private void GetBatchByid()
        {
            if (IdBatch != Guid.Empty)
            {
                var manager = new ProductionBatchesBLL();
                List<ProductionBatchesEL> list = manager.GetProductionBatchById(IdBatch);
                if (list.Count > 0)
                {
                    if (list[0].BatchStatus == 2)
                    {
                        BatchCompletionStatus = "Complete";
                    }
                    else
                    {
                        BatchCompletionStatus = "InComplete";
                    }
                    txtBatchCost.Text = list[0].BatchCost.ToString();
                }
            }
        }
        #endregion              
